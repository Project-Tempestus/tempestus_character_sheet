...
def build_html_elements():
    ...
    html_template = load_template(path="sheet_row.j2")
    for key, value in files_dict.items():
        yaml_text = load_yml(path="data/"+value)
        elements_dict[key] = render_template(template=html_template,data=yaml_text)
...
def build_html():
    ...
    html_template = load_template(path = "character_sheet.j2")
    html_content = render_template(template=html_template, data=elements_dict)
